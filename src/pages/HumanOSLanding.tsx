...
export default function HumanOSLanding() {
  ...
  return (
    <>
      {/* ── NAV ── */}
      <nav className="fixed top-0 left-0 right-0 z-50 h-[72px] flex items-center justify-between px-[clamp(1.5rem,5vw,4rem)] bg-navy/95 backdrop-blur-[12px] border-b border-gold/15">
        <Link to="/" className="font-display text-2xl font-bold text-white no-underline">
          M2M<span className="text-gold">~</span>Inc.
        </Link>
        <Link
          to="/"
          className="text-[0.8rem] text-white-dim no-underline flex items-center gap-1.5 font-mono tracking-[0.05em] hover:text-gold transition-colors"
        >
          ← All Platforms
        </Link>
        <button
      ...
      <footer className="bg-[#060F1E] py-10 px-[clamp(1.5rem,5vw,4rem)] border-t border-gold/10">
        ...
        <div className="max-w-[1280px] mx-auto mt-6 pt-5 border-t border-white/5 text-[0.72rem] text-white-dim flex justify-between flex-wrap gap-2">
          <span>
            © 2026 M2M~Inc. Human OS™, PIVOT OS™, BRIDGE OS™, M2M Composure
            Architecture™, C.A.L.M.™, M2M-AP™, M2M-PP™, M2M-EP™ are pending
            trademarks.
          </span>
